...
avg_and_std <- gathered_data %>% group_by(sample_number, volume_ratio, particle_size_microns, mass_ratio) %>% summarise(avg = mean(uH), dev = sd(uH))
...
ggplot(avg_and_std,aes(x = mass_ratio, y = avg, group = particle_size_microns, color = as.factor(particle_size_microns))) + geom_line() + geom_point() +
  
  labs(title = "Graph of Inductance against mass ratio", subtitle  = "Each line represents particles of different sizes", color = "Iron particle sizes") + 
  xlab(label = "Percentage of Iron in sample by mass") + 
  ylab(label = "Inductance measured at 1KHz") +
  geom_errorbar(aes(ymin=avg-dev, ymax=avg+dev), width=.01)

# to do
# faceted version of by mass ratio

# to do
# create function that takes data and title, label parameters and creates graphs
```
